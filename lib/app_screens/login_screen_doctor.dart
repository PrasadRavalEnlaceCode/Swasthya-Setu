    ...
    if (mobNoToValidate == "") {
      ...
      try {
        if (model.status == "OK") {
          ...
          if(jsonData[0]['ScreenType'] == 'doctor') {
          ...
          if (jsonData[0]['ScreenType'] == 'frontoffice') {
            debugPrint(jsonData[0]['ScreenType']);

            // *********
            // here staffidp is stored as doctoridp
            // and doctoridp is stored as patientidp

            var staffIDP = jsonData[0]['FrontOfficeIDP'].toString();
            setDoctorIDP(staffIDP);
            var patientIDP = jsonData[0]['DoctorIDF'].toString();
            setPatientIDP(patientIDP);
            setMobNo(jsonData[0]['FrontOfficeMobileNo']);


            var patientUniqueKey = ("${jsonData[0]['DoctorIDF']}_${jsonData[0]['FrontOfficeMobileNo']}");
            setPatientUniqueKey(patientUniqueKey);
            Navigator.pushReplacement(context,
                MaterialPageRoute(
                    builder: (context) => ReceptionDashboardScreen()));
          }
          else if (jsonData[0]['ScreenType'] == 'nursing') {
            debugPrint(jsonData[0]['ScreenType']);
            ...
            var patientUniqueKey = decodeBase64(jsonData[0]['StaffUniqueKey']);
            setPatientUniqueKey(patientUniqueKey);
            // var patientUniqueKey = ("${jsonData[0]['DoctorIDF']}_${jsonData[0]['FrontOfficeMobileNo']}");
            // setPatientUniqueKey(patientUniqueKey);
            Navigator.pushReplacement(context,
          ...
          else if (jsonData[0]['ScreenType'] == 'doctor') {

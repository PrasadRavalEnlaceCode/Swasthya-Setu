...
class AddPatientReportScreen extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return AddPatientReportScreenState();
  }
}

class AddPatientReportScreenState extends State<AddPatientReportScreen> {
  ...
  void initState() {
    ...
    tagNameController = TextEditingController(text: "Document-$formatted");
              ...
              children: <Widget>[
                ...
                Expanded(
                  ...
                  children: <Widget>[
                      ...
                      children: <Widget>[
                        GestureDetector(
                          onTap: () async {
                            ...
                            //openFilesAndSubmitSelected(context);
                          },
                              ...
                              Row(
                                  ...
                                  children: <Widget>[
                                    Container(
                                      ...
                                      child: widget.selectedFileType == "image"
                                          ? Image(
                                              fit: BoxFit.fill,
                                              image: FileImage(widget.image),
                                              height:
                    ...
                    widget.from != "prescription_fixed"
                        ? Align(
                            alignment: Alignment.center,
  ...
  void submitPatientReport(BuildContext context, image) async {
    ...
    if (categoryController.text.trim() == "") {
      final snackBar = SnackBar(
        backgroundColor: Colors.red,
        content: Text("Please type Document Type"),
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
    ...
    if (entryDateController.text == "") {
      final snackBar = SnackBar(
        backgroundColor: Colors.red,
        content: Text("Please select Document Date"),
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
    ...
    if (entryTimeController.text == "") {
      final snackBar = SnackBar(
        backgroundColor: Colors.red,
        content: Text("Please select Document Time"),
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
    ...
    final multipartRequest = MultipartRequest(
      'POST',
      Uri.parse("${baseURL}patientReportSubmit.php"),
    ...
    if (image != null) {
      ...
      multipartRequest.files.add(new http.MultipartFile(
          'ReportImage', image.openRead(), imgLength,
          filename: image.path));
    }
  ...
  void getCategoryList(BuildContext context) async {
    ...
    if (model.status == "OK") {
      ...
      debugPrint("Selected Category IDP :- ");
      debugPrint(selectedCategoryIDP);
      for (int i = 0; i < jsonData.length; i++) {
        final jo = jsonData[i];
        if (selectedCategoryIDP == jo['CategoryIDP']) {
          categoryController = TextEditingController(text: jo['CategoryName']);
          tagNameController = TextEditingController(
              text:
                  jo['CategoryName'].toString().substring(0, 2).toUpperCase() +
                      "-" +
                      entryDateController.text);
        }
        listCategories.add({
  ...
  void showCategorySelectionDialog(List<Map<String, String>> listCategories) {
    showDialog(
        barrierDismissible: false,
        context: context,
        builder: (BuildContext context) => Dialog(
                      ...
                      return InkWell(
                          onTap: () {
                            ...
                            categoryController = TextEditingController(
                                text: listCategories[index]["categoryName"]);
                            tagNameController = TextEditingController(
                                text: listCategories[index]["categoryName"]!
                                        .substring(0, 2)
                                        .toUpperCase() +
                                    "-" +
                                    entryDateController.text);
                            debugPrint("category idp - " + selectedCategoryIDP);
                            Navigator.of(context).pop();
                            setState(() {});
                          },
                          child: Padding(

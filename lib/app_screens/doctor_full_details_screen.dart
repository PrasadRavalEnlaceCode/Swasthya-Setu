...
class DoctorFullDetailsScreen extends StatefulWidget {
  Map<String, String> doctorData;
  String patientIDP, from;

  DoctorFullDetailsScreen(this.doctorData, this.patientIDP,
      {this.from = "doctorList"});

  @override
  State<StatefulWidget> createState() {
    return DoctorFullDetailsScreenState();
  }
}

class DoctorFullDetailsScreenState extends State<DoctorFullDetailsScreen> {
        ...
        return ListView(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: widget.from == "doctorList"
                      ...
                      : InkWell(
                          onTap: () {
                            bindRequestToDoctor(widget.doctorData);
                          },
                          child: Card(
                            elevation: 0,
                            shadowColor: Colors.grey,
            ...
            widget.from == "doctorList"
                ? Column(
                    children: [
                      widget.doctorData.containsKey("DueAmount") &&
                              double.parse(widget.doctorData["DueAmount"]!) > 0
                          ? Container(
                              padding: EdgeInsets.all(10),
                              decoration: BoxDecoration(
                                color: colorWhite,
  ...
  isImageNotNullAndBlank() {
    return (widget.doctorData["DoctorImage"] != "" &&
        widget.doctorData["DoctorImage"] != null &&
        widget.doctorData["DoctorImage"] != "null");
  }
  ...
  void getPermissionsData(BuildContext context) async {
    ...
    if (model.status == "OK") {
      ...
      debugPrint("Decoded Data Array : " + strData);
      final jsonData = json.decode(strData);
      widget.doctorData["HealthRecordsDisplayStatus"] =
          jsonData[0]['HealthRecordsDisplayStatus'];
      widget.doctorData["ConsultationDisplayStatus"] =
  ...
  void getDuePayment(
    ...
    if (model.status == "OK") {
      ...
      widget.doctorData["DueAmount"] = dueAmount;

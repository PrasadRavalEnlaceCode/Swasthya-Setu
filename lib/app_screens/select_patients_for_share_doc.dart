...
class SelectPatientsForShareDocument extends StatefulWidget {
  ModelHealthDoc? DocumentModel;
  String doctorName;
  GlobalKey<ProgressDialogWithPercentageState> progressKey = GlobalKey();

  SelectPatientsForShareDocument(this.DocumentModel, this.doctorName);

  @override
  State<StatefulWidget> createState() {
    return SelectPatientsForShareDocumentState();
  }
}

class SelectPatientsForShareDocumentState
    extends State<SelectPatientsForShareDocument> {
  Icon icon = Icon(
    Icons.search,
    color: Colors.black,
  );
    ...
    if (_radioValueSendType == 0) {
      if (listCategoriesSelected.length > 0) {
        if (widget.DocumentModel == null) {
          ...
          submitPatientsForDocumentShare(context, listCategoriesSelected);
      } else {
        final snackBar = SnackBar(
          backgroundColor: Colors.red,
      ...
      if (listPatientsSelected.length > 0) {
        if (widget.DocumentModel == null) {
          ...
          submitPatientsForDocumentShare(context, listPatientsSelected);
      } else {
        final snackBar = SnackBar(
          backgroundColor: Colors.red,
  ...
  void submitPatientsForDocumentShare(
    ...
    String loginUrl = "${baseURL}doctorSendDocumentNotification.php";
    ProgressDialog? pr;
    Future.delayed(Duration.zero, () {
      pr = ProgressDialog(context);
      pr!.show();
    ...
    String jsonStr = "{" +
        ...
        ",\"DoctorIDP\":\"$doctorIDP\"" +
        ",\"DocumentIDP\":\"${widget.DocumentModel!.healthInfoDocumentIDP}\"" +
